...
public class NoticeController {
    ...
    @PostMapping
    public Result<Notice> create(@Valid @RequestBody Notice input, HttpServletRequest req) {
        ...
        store.persist();
        store.addLog(null, currentUser(req).username(), "发布公告: " + notice.title(), "通知公告");
        return Result.ok(notice);
    }

    @PutMapping("/{id}")
    public Result<Notice> update(@PathVariable Long id, @Valid @RequestBody Notice input, HttpServletRequest req) {
        ...
        store.persist();
        store.addLog(null, currentUser(req).username(), "修改公告: " + notice.title(), "通知公告");
        return Result.ok(notice);
    }
    ...
    public Result<Void> delete(@PathVariable Long id, HttpServletRequest req) {
        ...
        store.persist();
        store.addLog(null, currentUser(req).username(), "删除公告: " + removed.title(), "通知公告");
        return Result.ok();
    }
