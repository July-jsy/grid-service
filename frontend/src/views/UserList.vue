...
const form = reactive({ username: '', displayName: '', role: '网格员', phone: '', status: '启用' })
...
<template>
  ...
  <div v-if="show" class="modal-mask" @click.self="show = false"><form class="modal" @submit.prevent="save"><div class="panel-head"><h3>新增用户</h3><button type="button" class="ghost" @click="show = false">关闭</button></div><label>用户名<input v-model="form.username" required /></label><label>姓名<input v-model="form.displayName" required /></label><label>角色<select v-model="form.role"><option>管理员</option><option>网格员</option><option>普通用户</option></select></label><label>联系电话<input v-model="form.phone" /></label><button class="primary wide">保存</button></form></div>
</template>
