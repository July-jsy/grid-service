...
const form = reactive({ title: '', category: '', gridName: '', reporter: '居民上报', description: '' })
...
<template>
  <section class="panel">
    <div class="panel-head"><div><p class="eyebrow">EVENT CENTER</p><h3>事件闭环处置</h3></div><button class="primary" @click="show = true">上报事件</button></div>
    ...
    <div class="event-list"><article v-for="row in rows" :key="row.id" class="event-card"><div><span class="tag">{{ row.category }}</span><small>{{ row.code }}</small><h3>{{ row.title }}</h3><p>{{ row.description }}</p><span class="muted">{{ row.gridName }} · {{ row.reporter }} · {{ format(row.createdAt) }}</span></div><div class="event-action"><span :class="['status', `status-${row.status}`]">{{ row.status }}</span><select :value="row.status" @change="change(row, $event.target.value)"><option v-for="item in statuses" :key="item">{{ item }}</option></select></div></article></div>
  </section>
  <div v-if="show" class="modal-mask" @click.self="show = false"><form class="modal" @submit.prevent="save"><div class="panel-head"><h3>上报事件</h3><button type="button" class="ghost" @click="show = false">关闭</button></div><label>事件标题<input v-model="form.title" required /></label><label>事件分类<select v-model="form.category" required><option disabled value="">请选择</option><option>城市管理</option><option>公共安全</option><option>环境卫生</option><option>矛盾纠纷</option><option>民生服务</option></select></label><label>所属网格<input v-model="form.gridName" required /></label><label>事件来源<input v-model="form.reporter" /></label><label>详细描述<textarea v-model="form.description"></textarea></label><p v-if="error" class="error">{{ error }}</p><button class="primary wide">提交并自动派单</button></form></div>
</template>
