...
const applyForm = reactive({ itemName: '', applicant: '', phone: '', materialsNote: '' })
...
<template>
  <section class="panel">
    <div class="panel-head"><div><p class="eyebrow">PUBLIC SERVICE</p><h3>便民服务事项</h3></div><div><button class="ghost" @click="applyShow = true">在线申办</button> <button class="primary" @click="show = true">新增事项</button></div></div>
  ...
  <section class="panel section-gap"><div class="panel-head"><h3>服务申请进度</h3><span>{{ applications.length }} 条申请</span></div><div class="table-wrap"><table><thead><tr><th>申请编号</th><th>事项</th><th>申请人</th><th>联系电话</th><th>状态</th><th>流转</th></tr></thead><tbody><tr v-for="row in applications" :key="row.id"><td>{{ row.code }}</td><td>{{ row.itemName }}</td><td>{{ row.applicant }}</td><td>{{ row.phone }}</td><td><span class="status">{{ row.status }}</span></td><td><select :value="row.status" @change="changeApply(row, $event.target.value)"><option>待受理</option><option>办理中</option><option>已办结</option></select></td></tr></tbody></table></div></section>
  ...
  <div v-if="applyShow" class="modal-mask" @click.self="applyShow = false"><form class="modal" @submit.prevent="apply"><div class="panel-head"><h3>在线申办</h3><button type="button" class="ghost" @click="applyShow = false">关闭</button></div><label>申请事项<select v-model="applyForm.itemName" required><option disabled value="">请选择</option><option v-for="row in rows" :key="row.id">{{ row.name }}</option></select></label><label>申请人<input v-model="applyForm.applicant" required /></label><label>联系电话<input v-model="applyForm.phone" /></label><label>材料说明<textarea v-model="applyForm.materialsNote"></textarea></label><button class="primary wide">提交申请</button></form></div>
</template>
